...
def constructBayesNet(gameState: hunters.GameState):
    ...
    ghost0_obs0 = (GHOST0, OBS0)
    pac_obs0 = (PAC, OBS0)
    ghost1_obs1 = (GHOST1, OBS1)
    pac_obs1 = (PAC, OBS1)

    edges.append(ghost0_obs0)
    edges.append(pac_obs0)
    edges.append(ghost1_obs1)
    edges.append(pac_obs1)
    ...
    # variableDomainsDict[OBS0] = [manhattanDistance(pacPos, ghost0Pos) for pacPos in variableDomainsDict[PAC] for ghost0Pos in variableDomainsDict[GHOST0]]
    # variableDomainsDict[OBS1] = [manhattanDistance(pacPos, ghost1Pos) for pacPos in variableDomainsDict[PAC] for ghost1Pos in variableDomainsDict[GHOST1]]

    variableDomainsDict[OBS0] = [i for i in range(manhattanDistance((0, 0), (X_RANGE, Y_RANGE)) + 6)]
    variableDomainsDict[OBS1] = [i for i in range(manhattanDistance((0, 0), (X_RANGE, Y_RANGE)) + 6)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # while there are hidden variables -> loop through elimination order
        for elimVar in eliminationOrder:

        # pick a variable to elim = elimVar
        # join all initial factors with factors mentioning variable to elim
            currentFactorsList, joined = joinFactorsByVariable(currentFactorsList, elimVar)

        # eliminate variable
            if len(joined.unconditionedVariables()) != 1:
                joined = eliminate(joined, elimVar)
                currentFactorsList.append(joined)

        # join remaining and normalize
        fullJoint =  joinFactors(currentFactorsList)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return random.choice(list(self.keys()), weights = list(self.values()))
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        obs_given_trueDistance = busters.getObservationProbability(noisyDistance, trueDistance)

        return obs_given_trueDistance
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in allGhostPositions:
            updatedProb = self.getObservationProb(observation, pacmanPosition, position, jailPosition)
            self.beliefs[position] *= updatedProb
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPosition in allGhostPositions:
            ...
            for newPosition in allGhostPositions:
                self.beliefs[newPosition] += newPosDist[newPosition] * oldBeliefs[oldPosition]
